    ...
    for i in range(iterationVal):
        with open(file, 'rb') as csvfile:
            ...
            for row in currentYear:
                ...
                #if we are within the week of chrimstas, multiply the probabilities by 3
                if (int(row[dayCol]) >= 18 or int(row[dayCol])<=31):
                    cancelProb = 3*setCancelProb
                    delayProb = 3*setDelayProb
                else:
                    cancelProb = setCancelProb
                    delayProb = setDelayProb                    
